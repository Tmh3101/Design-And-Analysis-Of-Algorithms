...
void BranchAndBound(DoVat *dsdv, int n, int i, float *V, float *TGT, float *CT, float *GLNTT, int *x){
	int yk = ((*V / dsdv[i].TL) < dsdv[i].SL) ? (*V / dsdv[i].TL) : dsdv[i].SL; // so vat i lon nhat co the cho vao balo
	int j;
	for(j = yk; j >= 0; j--){
...
int main(){
	...
	for(i = 0; i < n; i++){
		printf("%5.2f %5.2f %3d %5.2f %-20s\n", dsdv[i].TL, dsdv[i].GT, dsdv[i].SL, dsdv[i].DG, dsdv[i].tenDV);
	}
	printf("\n");
	...
	for(i = 0; i < n; i++){
		printf("%5.2f %5.2f %3d %5.2f %-20s %3d\n", dsdv[i].TL, dsdv[i].GT, dsdv[i].SL, dsdv[i].DG, dsdv[i].tenDV, dsdv[i].PA);
	}
	printf("\n");
